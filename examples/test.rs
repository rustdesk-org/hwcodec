...
fn test_encode_decode(encode_ctx: EncodeContext, decode_ctx: DecodeContext) {
    ...
    if args.len() < 2 {
        println!("usage:<exe> yuvfile");
    }
    let mut video_encoder = Encoder::new(encode_ctx.clone()).unwrap();
    let mut video_decoder = Decoder::new(decode_ctx.clone()).unwrap();
    let yuv_filename = args[1].to_string();
    std::fs::create_dir("output").ok();
    let mut yuv_file = File::open(yuv_filename).unwrap();
