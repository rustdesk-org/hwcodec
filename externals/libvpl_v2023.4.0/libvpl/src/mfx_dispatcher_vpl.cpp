...
mfxStatus MFXSetConfigFilterProperty(mfxConfig config, const mfxU8 *name,
                                     mfxVariant value) {
  if (!config)
...
mfxStatus MFXEnumImplementations(mfxLoader loader, mfxU32 i,
  ...
  if (loaderCtx->m_bNeedFullQuery) {
    // if a session was already created in low-latency mode, unload all
    // implementations
